...
fn main() {
    let input: i64 = std::io::stdin().lock()
                                     .lines()
                                     .next().unwrap().unwrap()
                                     .parse().unwrap();

    println!("{}", bootstrapping(input as f64).1);
}

fn bootstrapping(input: f64) -> (f64, f64) {
    search(binary_float_6_decimals,
           |x: &f64| x.powf(*x) > input,
           0.0,
           10_000.0)
}

// zero in on a floating point number with a tolerance of 1e-6
fn binary_float_6_decimals(left : &f64,
                           right: &f64) -> Option<f64>
{
...
mod tests {
    ...
    #[test] fn test_bootstrapping_1()
    {
        assert!(bootstrapping(4.0).1 - 2.0 < 0.000001);
    }

    #[test] fn test_bootstrapping_2()
    {
        assert!(bootstrapping(42.0).1 - 3.20720196137 < 0.000001);
    }
